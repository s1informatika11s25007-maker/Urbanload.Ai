  ...
  return (
    <Card className="p-4 sm:p-6 border-slate-200 bg-white shadow-sm rounded-2xl space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {zones.length === 0 ? (
          <div className="col-span-1 md:col-span-2 p-6 text-center text-xs text-slate-400">
          ...
          zones.map((z) => (
            <div key={z.id} className="p-3.5 bg-slate-50 border border-slate-200/80 rounded-xl flex items-center justify-between">
              <div>
                <span className="font-bold text-xs sm:text-sm text-slate-900 block">{z.name}</span>
                <span className="text-[10px] text-slate-500 font-mono">Tipe: {z.zone_type || 'logistics'} | Prioritas: {z.priority_level || 'normal'}</span>
              </div>
              <span className="text-xs font-black text-teal-700 bg-teal-100/60 px-2.5 py-1 rounded-lg border border-teal-200 shrink-0">
